...
var riotChamp = require('./riot.js')({
	type: "champion",
	key: credentials.riot.key,
	region: 'na'
});

var riotItem = require('./riot.js')({
	type: "item",
	key: credentials.riot.key,
	region: 'na'
});
...
module.exports = {
	...
	createParticipantsTemplate: function(match) {
		...
		match.participants.forEach(function(participant){
			...
			if(timeline.csDiffPerMinDeltas) newParticipant.inGameStats.csDiffPerMin = timeline.csDiffPerMinDeltas;
				...
				async.series([
					...
					function(innerCb) {
						...
						// otherwise make a new champion object to push into the database
							...
							riotChamp.get(participant.championId, function(data) {
								var loadImgPath = data.image.full.substr(0, data.image.full.indexOf('.')) + '_0.jpg';
								dbChamp = new Champion({
									championId: participant.championId,
									name: data.name,
									img: 'http://ddragon.leagueoflegends.com/cdn/img/champion/loading/' + loadImgPath,
									sprite: 'http://ddragon.leagueoflegends.com/cdn/5.2.1/img/champion/' + data.image.full,
									regionalStats: []
								});
								innerCb();
							});	
					...
					function(innerCb) {
						...
						participant.items.forEach(function(item){
							if(item != 0) addMatch(dbRegion.items, "itemId", item);
						});
						if(participant.brawlerId != 0) addMatch(dbRegion.brawlers, "brawlerId", participant.brawlerId);
						...
						if(participant.inGameStats.csDiffPerMin) {
				...
				async.series([
					...
					function(innerCb) {
						...
						if(item !== undefined && item !== null) {
							...
							riotItem.get(playerItem, function(data) { 
								dbItem = new Item({
									itemId: playerItem,
									itemName: data.name,
									sprite: 'http://ddragon.leagueoflegends.com/cdn/5.2.1/img/item/' + data.image.full,
									regionalStats: []
								});
								innerCb();
							});
			...
			async.series([
				function(callBack) {
					async.eachSeries(participant.items, function(item, cb) {
						if(item != 0) updateItem(participant, item, cb);
						else cb();
					}, callBack);
				},
				function(callBack) {
					async.eachSeries(participant.brawlerUpgrades, function(brawlerUpgrade, cb) {
						if(brawlerUpgrade != 0) updateItem(participant, brawlerUpgrade, cb);
						else cb();
					}, callBack);
				},
				function(callBack) {
					if(participant.brawlerId != 0) updateItem(participant, participant.brawlerId, callBack);
					else callBack();
				},
			], loopCb);
		}, mainCallback);
