...
module.exports = {
	...
	saveParticipants: function(match) {
		var participantArr = [];
	...
	updateChampions: function(participants, newMatch) {
	...
	updateItems: function(participants, newMatch) {
		function updateItem(participant, item, playerItem) {
			var dbItem;
		...
		participants.forEach(function(participant) {
			participant.items.forEach(function(playerItem) {
				Item.findOne({ itemId: playerItem }, function(err, item) {
					if (err) return console.error(err);
					updateItem(participant, item, playerItem);
				});
			});
			participant.brawlerUpgrades.forEach(function(brawlerUpgrade) {
				Item.findOne({ itemId: brawlerUpgrade }, function(err, item) {
					if (err) return console.error(err);
					updateItem(participant, item, brawlerUpgrade);
				});
			});
			Item.findOne({ itemId: participant.brawlerID }, function(err, item) {
				if (err) return console.error(err);
				updateItem(participant, item, participant.brawlerID);
			});
		});
	}
};