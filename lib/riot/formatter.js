...
module.exports = {
	...
	updateItems: function(participants, newMatch) {
		function updateItem(participant, playerItem) {
		...
		participants.forEach(function(participant) {
			async.series([
				async.each(participant.items, function(item) {
					updateItem(participant, item);
				}),
				async.each(participant.brawlerUpgrades, function(brawlerUpgrade) {
					updateItem(participant, brawlerUpgrade);
				}),
				updateItem(participant, participant.brawlerID)
			]);
		});
	}
};