...
module.exports = {
	...
	updateItems: function(participants, newMatch) {
		...
		async.each(participants, function(participant, mainCb) {
			async.series([
				function(callBack) {
					async.each(participant.items, function(item, cb) {
						updateItem(participant, item, cb);
					}, callBack);
				},
				function(callBack) {
					async.each(participant.brawlerUpgrades, function(brawlerUpgrade, cb) {
