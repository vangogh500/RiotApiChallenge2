			...
			// otherwise make new match object and add it to the database
				...
				match.participants.forEach(function(participant){
					...
					Champion.findOne({ championId: participant.championId }, function(err, champion) {
						...
						dbRegion.picks += 1;
						console.log(dbChamp.regionalStats.filter(function(regionStats){ return regionStats.regionName == match.region }));
						// add one to wins parameter if participant won
						if(stats.winner){ dbRegion.wins += 1; }
				
						addMatch(dbRegion.lanes, "laneName", timeline.lane);
						console.log(dbRegion.lanes);
						addMatch(dbRegion.roles, "roleName", timeline.role);
						addMatch(dbRegion.ranks, "rankName", participant.highestAchievedSeasonTier);
						
						if(dbRegionalIdx === -1) dbChamp.regionalStats.push(dbRegion);
						else dbChamp.regionalStats[dbRegionalIdx] = dbRegion;
